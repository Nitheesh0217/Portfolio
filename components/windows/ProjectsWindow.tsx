            ...
            <div className="w-[180px] rounded-xl border border-white/10 bg-black/60 backdrop-blur-md p-3.5 flex flex-col gap-2">
              ...
              <div className="flex flex-col gap-1.5 mt-1">
                <div className="text-[8.5px] text-white/70 bg-white/5 px-2 py-1 rounded-lg self-end">
                  &ldquo;Analyze webhook retries&rdquo;
                </div>
...
export const ProjectsWindow = memo(function ProjectsWindow({ projects, state, onSelectProject }: ProjectsWindowProps) {
  ...
  return (
    <div className="w-full h-full flex flex-col relative select-none bg-transparent">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 pb-12">
          {filteredProjects.map((p) => (
            <div
              ...
              style={{ boxShadow: '0 12px 32px rgba(0,0,0,0.30), inset 0 1px 0 rgba(255,255,255,0.03)' }}
            >
              <div className="absolute inset-0 w-full h-full">
                {p.thumbnail_url ? (
                  // eslint-disable-next-line @next/next/no-img-element
                  <img src={p.thumbnail_url} alt={p.title} className="w-full h-full object-cover group-hover:scale-[1.03] transition-transform duration-500" />
                ) : (
      ...
      >
        <button
          ...
          className={`px-4 py-1.5 rounded-full text-[12px] font-semibold tracking-tight transition-all ${
            selectedCategory === '__all__' ? 'bg-white/25 text-white shadow-inner border border-white/5' : 'text-white/60 hover:text-white'
          }`}
        >
          All Works
        </button>
        {categories.map((cat) => (
          <button
            ...
            className={`px-4 py-1.5 rounded-full text-[12px] font-semibold tracking-tight transition-all whitespace-nowrap ${
              selectedCategory === cat ? 'bg-white/25 text-white shadow-inner border border-white/5' : 'text-white/60 hover:text-white'
            }`}
          >
            {categoryLabel(cat)}
          </button>
      ...
      {activeCaseStudy && (
        <div className="absolute inset-0 z-50 bg-[#0a0a0f]/95 backdrop-blur-xl flex flex-col animate-fade-in" style={{ animationDuration: '200ms' }}>
          <div className="flex items-center justify-between px-10 py-5 border-b border-white/[0.06] bg-black/20">
            <div>
              <div className="flex items-center gap-2">
                <span className="text-[10px] font-black uppercase tracking-widest text-amber-300">{categoryLabel(activeCaseStudy.category)}</span>
          ...
          <div className="flex-1 overflow-y-auto px-10 py-8" style={{ scrollbarWidth: 'thin' }}>
            <div className="grid grid-cols-1 lg:grid-cols-[1.3fr_1fr] gap-10 pb-12">
              <div className="flex flex-col gap-6">
                <div className="w-full aspect-video rounded-[24px] overflow-hidden border border-white/[0.06] relative">
                  {activeCaseStudy.thumbnail_url ? (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img src={activeCaseStudy.thumbnail_url} alt={activeCaseStudy.title} className="w-full h-full object-cover" />
                  ) : (
                ...
                <div className="flex flex-wrap gap-2">
                  {activeCaseStudy.stack.map((tech) => (
                    <span key={tech} className="text-[10px] font-mono font-bold text-amber-300 bg-amber-400/5 border border-amber-400/15 px-2.5 py-1 rounded-lg">{tech}</span>
                  ))}
                </div>
                <div className="flex gap-3">
                  {activeCaseStudy.live_url && (
                    <a href={activeCaseStudy.live_url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 flex-1 justify-center bg-gradient-to-r from-amber-400 to-amber-500 hover:brightness-105 active:scale-95 text-[#0e0d14] text-[12.5px] font-bold py-3 rounded-xl transition-all shadow-lg shadow-amber-400/10">
                      <ExternalLink className="w-4 h-4" /> View Live Project
                    </a>
                  )}
                  {activeCaseStudy.github_url && (
                    <a href={activeCaseStudy.github_url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 px-6 py-3 border border-white/[0.10] hover:border-white/[0.22] hover:bg-white/[0.02] text-white/70 hover:text-white text-[12.5px] font-semibold rounded-xl transition-all active:scale-95">
              ...
              <div className="flex flex-col gap-6">
                {activeCaseStudy.problem_statement && <NarrativeBlock icon={Target} label="The Challenge / Problem" text={activeCaseStudy.problem_statement} />}
                {activeCaseStudy.approach && <NarrativeBlock icon={Zap} label="The Solution / Approach" text={activeCaseStudy.approach} />}
                {activeCaseStudy.process_notes && <NarrativeBlock icon={BookOpen} label="Process Notes & Architecture" text={activeCaseStudy.process_notes} />}
                  ...
                  <div className="rounded-[24px] border border-emerald-500/25 bg-emerald-500/[0.04] p-5 relative overflow-hidden">
                    <div className="relative">
                      <div className="flex items-baseline gap-2">
                        <span className="text-[38px] font-black text-emerald-400 leading-none tracking-tight">{activeCaseStudy.roi_value}</span>
                        <CheckCircle2 className="w-4.5 h-4.5 text-emerald-400/60" />
                      </div>
                      <p className="text-[11px] font-bold text-emerald-300/80 mt-1 uppercase tracking-wider">{activeCaseStudy.roi_label}</p>
                      {activeCaseStudy.roi_context && <p className="text-[11px] text-white/40 mt-1.5 leading-relaxed font-sans">{activeCaseStudy.roi_context}</p>}
                    </div>
                  </div>
                )}
                    ...
                    <div className="flex items-start gap-4">
                      <div className="shrink-0 w-8 h-8 rounded-full flex items-center justify-center text-[10px] font-bold text-white bg-gradient-to-br from-amber-400/40 to-orange-500/40 border border-white/10 overflow-hidden">
                        {activeCaseStudy.testimonial_avatar ? (
                          // eslint-disable-next-line @next/next/no-img-element
                          <img src={activeCaseStudy.testimonial_avatar} alt={activeCaseStudy.testimonial_author} className="w-full h-full object-cover" />
                        ) : (
                          <span>{activeCaseStudy.testimonial_author.split(',')[0].split(' ').map((w) => w[0]).join('').slice(0, 2).toUpperCase()}</span>
                        )}
                      </div>
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[10px] text-white/35 mt-2.5 font-bold not-italic">— {activeCaseStudy.testimonial_author}</p>
                      </div>
                    </div>
                  </div>
                )}
